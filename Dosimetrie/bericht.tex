...
Eine Röntgenröhre (schematisch in Abbildung \ref{fig:roehre}) besteht aus einem evakuierten Glaskolben mit einer Anordnung von geheizter Kathode und Anode aus Targetmaterial.
...
\begin{itemize}
	...
	\item \textbf{Szintillationszähler, Halbleiterdetektoren, Geiger-Müller-Zählrohr:}\\
	Einfallende Teilchen erzeugen freie Ladungsträger, die als Stromimpuls (beim Szintillationsdetektor am Photomultiplier) gemessen werden können. Das in dem Versuch zum Einsatz kommende Geiger-Müller-Zählrohr wird im folgenden Abschnitt konkreter vorgestellt.
\end{itemize}
...
Unter Annahme eines nicht paralysierenden Systems mit der Totzeit $\tau$, der wahren Count-Rate $m$ und der vom Detektor gezählten Anzahl der Events $k$ in einer Zeit $T$ ergeben sich die folgenden Überlegungen \cite{leo}.
...
\begin{figure}[ht]
	\centering
	\input{./plots/theo_totzeitkorrektur.tex}
...
Die Ionendosis bezeichnet die freie elektrische Ladung $\mathrm{d}Q$, die durch ionisierende Strahlung in einer bestimmten Massenelement $\mathrm{d}m$ entsteht:
\begin{align}
...
\end{align}
oder für die Ionendosisleistung \ref{sec:ionendosis}:
\begin{align}
...
In einem Halter ist ein Film eingesetzt, der durch Filter in verschiedene Bereiche eingeteilt ist, welche durch unterschiedliche Dicken/Dichten den dynamischen Bereich des Dosimeters erweitern.
Die Filter erlauben eine Unterscheidung verschiedener Strahlungstypen, was es ermöglicht die Äquivalentdosis zu berechnen.
...
\subsubsection{Thermolumineszenzdosimeter}
...
\subsubsection{Elektronische Personendosimeter}
Als elektronisches Personendosimeter werden elektronische Geräte zusammengefasst, die mithilfe von Halbleiterdetektoren (PIN-Diode, MOSFETs) die Energie von einfallender Strahlung bestimmen.
\begin{itemize}
...
\end{align}
Da die auftretenden Ströme in der Größenordnung einiger \si{nA} liegen, muss ein mentsprechend hochohmiger Widerstand verwendet werden, was zur Folge hat, dass die Spannung nicht mit einem einfachen Voltmeter gemessen werden kann, da diese einen geringeren Innenwiderstand haben als der verwendete Messwiderstand.
...
\end{figure}
Man sieht sofort, dass sich der Verlauf des Ionisationsstroms in Abhängigkeit der Kondensatorspannung in zwei charakteristische Bereiche einteilen lässt.
...
Außerhalb des Messbereichs ($\SI{0}{V} \leq U_\mathrm{C} \leq \SI{400}{V}$) sind außerdem noch weitere Bereiche zu erwarten.
So können beispielsweise bei noch höheren Kondensatorspannungen ionisierte Teilchen im elektrischen Feld soviel Energie gewinnen, dass diese weitere Ionen erzeugen, was zu einem verstärkten Ionisationsstrom führt (Proportionalitätsbereich).
...
\begin{figure}[hp]
	\begin{subfigure}[b]{1\textwidth}
		...
		\subcaption{Molybdän-Röhre mit Anpassung einer linearen Funktion: \\$I_\mathrm{C} = \SI{4.97 +- 0.06}{\nano\ampere\per\milli\ampere} \cdot I_\mathrm{E} + \SI{0.12 +- 0.03}{nA}$}
	...
	\begin{subfigure}[b]{1\textwidth}
		...
		\subcaption{Kupfer-Röhre mit starker Abweichung vom erwarteten linearen Verhalten: \\$I_\mathrm{C} = \SI{94.5 +- 0.9}{\nano\ampere\per\milli\ampere} \cdot I_\mathrm{E} - \SI{42.9 +- 1.1}{\nano\ampere\per\milli\ampere\squared} \cdot I_\mathrm{E}^2$}
...
Weiterhin ist für die Kupfer-Röhre in Abbildung \ref{fig:cu_roehrenspannung} erneut ein Abflachen des Ionisationsstroms bei hohen Röhrenspannungen zu beobachten, was abermals auf Raumladungseffekte zurückzuführen sein könnte.
...
Der Grund könnte sein, dass im Kondensator soviele Ionisationen stattfinden, dass die stationäre Raumladung für einen Sättigung der Ionisationskammer sorgt.
...
\end{align*}
Die Dosisleistung $\dot{D}$ kann nun durch Division durch die Bestrahlungszeit $t$ berechnet werden, wobei der Fehler durch Gauß'sche Fehlerfortpflanzung durch:
\begin{align}
...
Die Messung zwei nahezu identischer Dosisleistungen könnte auf einen Sättigungseffekt des Stabdosimeters zurückzuführen sein, wobei die Quarzfaser in beiden Fällen noch auf der Skala (0 bis \SI{10}{mSv}) sichtbar war.
...
\end{align*}
Auffällig ist die starke Abweichung vom Abstandsquadratsgesetz bei kleinen Abständen von der Anode.
...
Unter Berücksichtigung dieses Effekts zeigt die Anpassung eine gute Übereinstimmung mit dem Abstandsquadratgesetz.
Die Notwendigkeit eines konstanten Versatzes $D_0$ für eine gute Anpassung könnte ebenfalls auf geometrische Effekte zurückzuführen sein, wie z.B. die Abweichung von einer idealen Punktquelle oder den Weglängenunterschied (jeweils von der Quelle) zwischen der Mitte der Ionisationskammer und des oberen/unteren Randes.
...
Zur Messung wird die Zählrate über $t=\SI{30}{\second}$ gemittelt und eine Röhrenspannung von $U=\SI{35}{\kilo\volt}$ fest eingestellt, während der Emissionsstrom $I_\text{E}$ von \num{0} bis \SI{1}{\milli\ampere} variiert wird.
...
Die Messungen zu diesem Abschnitt wurden mit den gleichen Einstellungen wie oben durchgeführt ($t=\SI{30}{\second}, U=\SI{35}{\kilo\volt}$, maximale Zählrate bei $\beta=\SI{-0.9}{\degree}$).
...
\begin{figure}[hbt]
	\centering
	\input{./plots/abschirmung/absorber_1_log.tex}
...
\begin{table}[ht]
	\centering
	\resizebox{\textwidth}{!}{
	\input{./tables/absorber/elemente.tex}
	}
...
\begin{align*}
d &= \SI{0.60+-0.13}{cm} \qquad\text{(ohne Zirkonfilter)}\\
...
\begin{table}[ht]
	\centering
	\resizebox{\textwidth}{!}{
	\input{./tables/absorber/koeffizienten.tex}
	}
	\caption{\korr{Neu} Transmissionskoeffizienten für die verschiedenen Absorberelemente jeweils mit und ohne Zirkonfilter sowie die gemittelten Transmissionskoeffizienten.}
...
\begin{figure}[ht]
	\centering
	\input{./plots/abschirmung/koeffizienten.tex}
	\caption{\korr{Neu} Transmissionskoeffizienten in Abhängigkeit der Ordnungszahl des abschirmenden Elements.  Aufgrund der bekannten schlechten Ergebnisse ist diese Abbildung nicht geeignet, um den zu zeigenden Zusammenhang zu erkennen.}
...
Der Versuchsteil zur Abschirmung hingegen konnte nicht erfolgreich durchgeführt werden.
