...
\begin{abstract}
\noindent
In dem vorliegenden Versuch soll ein Helium-Neon Experimentierlaser aufgebaut werden und auf seine Eigenschaften hin untersucht werden.
Dabei soll das Strahlprofil und die Polarisation untersucht und die Wellenlänge bestimmt werden.
Weiterhin wird das Modenverhalten des Lasers analysiert - zum Einsatz kommen dazu ein optischer Spektrumanalysator und eine optische Schwebung, deren Aufbau und Wirkung verstanden werden soll.
\end{abstract}
...
Für den Fall, dass das einfallende Licht bereits linear polarisiert ist (wie für den hier verwendeten Laser angenommen wird), gilt für die hinter dem Polarisator registrierte Intensität (Gesetz von Malus):
\begin{align}
...
Dieser ist gleichzeitig der Abstand von der Strahltaille, da diese bei halbsymmetrischen Resonatoren auf dem ebenen Spiegel liegt (siehe auch Abschnitt \ref{ssec:halbsym_res}).\\
\\
...
Die angesprochenen Reflexionen an der optischen Diode führen weiterhin zu einer unerwünschten Rückkopplung des Strahls in den Resonator, was sich allerdings nicht vermeiden ließ.\textbf{Durch Drehen konnte die doch vermieden werden?}
...
\caption{Beobachtetes Modenspektrum und Anpassung für die Resonatorlänge \SI{69+-0.4}{\centi\meter} (Datensatz ALL0026)}
\label{fig:ALL0026}
\end{figure}
\begin{table}[hb]
\centering
\resizebox{0.95\columnwidth}{!}{%
\input{./tables/modenabstand/ALL0026.tex}}
\caption{Messwerte und Berechnung zum longitudinalen Modenabstand. $x_i$ beschreibt die Position der Maxima, $\Delta T$ die Differenz zweier aufeinander folgender Positionen und $T_\text{konf.}$ den Abstand zweier gleicher Tranversalmoden. Dieser ist getrennt dargestellt, da die Werte keinen zeilenweisen Bezug zu den $x_i$ haben.}
\end{table}
\clearpage
\begin{figure}[ht]
...
\caption{Beobachtetes Modenspektrum und Anpassung für die Resonatorlänge \SI{79.5+-0.4}{\centi\meter} (Datensatz ALL0022)}
\label{fig:ALL0022}
\end{figure}
\begin{table}[hb]
\centering
\resizebox{0.95\columnwidth}{!}{%
\input{./tables/modenabstand/ALL0022.tex}}
\caption{Messwerte und Berechnung zum longitudinalen Modenabstand. $x_i$ beschreibt die Position der Maxima, $\Delta T$ die Differenz zweier aufeinander folgender Positionen und $T_\text{konf.}$ den Abstand zweier gleicher Tranversalmoden. Dieser ist getrennt dargestellt, da die Werte keinen zeilenweisen Bezug zu den $x_i$ haben.}
\end{table}
\clearpage
\begin{figure}[ht]
...
\caption{Beobachtetes Modenspektrum und Anpassung für die Resonatorlänge \SI{89.2+-0.4}{\centi\meter} (Datensatz ALL0014)}
\label{fig:ALL0014}
\end{figure}
\begin{table}[hb]
\centering
\resizebox{0.95\columnwidth}{!}{%
\input{./tables/modenabstand/ALL0014.tex}}
\caption{Messwerte und Berechnung zum longitudinalen Modenabstand. $x_i$ beschreibt die Position der Maxima, $\Delta T$ die Differenz zweier aufeinander folgender Positionen und $T_\text{konf.}$ den Abstand zweier gleicher Tranversalmoden. Dieser ist getrennt dargestellt, da die Werte keinen zeilenweisen Bezug zu den $x_i$ haben.}
\end{table}
\clearpage
