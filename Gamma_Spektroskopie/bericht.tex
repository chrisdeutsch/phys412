...
\begin{itemize}
	...
	Die Breite einer solchen Linie wird durch statistische Fluktuationen in der Anzahl der Ionisationen/Anregungen $N$ im Detektormaterial durch das detektierte Teilchen.
...
Der Kristall ist aufgrund des geringen Konzetrationsgrades der Dotierung für diese Photonen transparent und kann durch einen Photomultiplier verstärkt werden.
...
Da Elektron-Loch-Paare aufgrund der geringen Bandlücke bei Halbleitern bei geringeren Energien produziert werden, werden so insgesamt mehr Paare erzeugt und die Zählrate erhöht.
Durch die im Vergleich zum Gas-Ionisationsdetektor höhere Dichte wird außerdem mehr Energie im Halbleiterdetektor deponiert, was angestrebt ist.
...
Durch den Übergang zwischen p- und n-Schicht entsteht eine Verarmungszone, in der die Elektron-Loch-Paare durch einfallende Strahlung erzeugt werden.
Das Anlegen einer Spannung in Sperrrichtung bewirkt sowohl eine Vergrößerung der Verarmungszone, in der dann mehr Strahlung detektiert wird und erhöhen das elektrische Feld in der Verarmungszone, wodurch eine bessere Ladungssammlung ermöglicht wird.
...
Ein eintreffendes Teilchen einer bestimmten Energie erzeugt ein logisches Signal, so dass der Wert in dem zu dieser Energie korrespondieren Kanal um eins erhöht wird.
Alle auf den Detektor treffenden Teilchen werden somit nach ihrer Energie histogrammiert, die entstehende Darstellung wird dann als (Gamma-)Spektrum bezeichnet.
...
\end{figure}
Die durch $\gamma$-Spektroskopie aufgenommenen Spektren weisen einige charakteristische Merkmale (Abbildung \ref{fig:schema_spektrum}) auf, die durch die Interaktion von Photonen mit Materie entstehen:
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item Eines der entstandenen Photonen $E_\gamma = \SI{511}{keV}$ interagiert mit dem Detektormaterial durch den Photoeffekt und deponiert seine gesamte Energie im Detektor.
		...
		\item Fall 3:
		Beide Photonen werden im Detektormaterial absorbiert, wodurch die gesamte Energie des eingefallenen $\gamma$-Quants im Detektor deponiert wird.
...
\begin{figure}[ht]
	\centering
	\input{./plots/szintillator/rueckstreupeaks/caesium.tex}
...
\begin{table}[ht]
	\centering
	\input{./tables/rueckstreupeaks_szinti.tex}
...
\begin{table}[h]
	\centering
	\input{./tables/abstaende_halbleiterdetektor.tex}
...
\begin{figure}[ht]
	\centering
	\input{./plots/halbleiter/rueckstreupeaks/caesium.tex}
...
\begin{table}[ht]
	\centering
	\input{./tables/rueckstreupeaks_halb.tex}
...
\begin{table}[ht]
	\centering
	\resizebox{\textwidth}{!}{
	\input{./tables/effizienz_halb.tex}
	}
...
\begin{figure}[hp]
	...
	\begin{subfigure}[b]{0.65\textwidth}
		\resizebox{!}{0.28\textheight}{
			\input{./plots/langzeitmessung/probe.tex}
		}
...
\begin{sidewaystable}[hp]
	\centering
	\input{./tables/langzeitmessung.tex}
...
\begin{sidewaysfigure}
		\centering
		\input{./plots/langzeitmessung/probe_gross.tex}
