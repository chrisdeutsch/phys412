...
\begin{itemize}
	\item Beschreibung der Schiebe-Messmethode und der auftretenden Fehler. Wo liegt die Strahltaille?
	
	\item Erklärung was eigentlich gemessen wird. Messschieber schneidet den Gaußstrahl ab, sodass ein proportionaler Radius gemessen wird. Gleichgewicht zwischen Verstärkung und Verluste erklären.
	Wie kommt man physikalisch auf die Fithypothese.
	
	\item Anpassung an die Datenpunkte erklären. Hat der Proportionalitätsfaktor Abhängigkeiten von den anderen Fitparametern? Vermutlich schon, aber nur Schwache.
	
	Fithypothese: $w_\mathrm{fit}(z) = \alpha \cdot w_\mathrm{exp.}$, wobei der experimentelle Strahlradius gegeben ist durch:
	\begin{align}
	w_\mathrm{exp.}(z) = w_0 \sqrt{1 + \left( \frac{\lambda}{\pi} \right)^2 \left( \frac{z}{w_0^2}\right)^2}
	\end{align}
	Fit erfolgt dann über $\alpha$ und $w_0$.
...
Die Messung der Strahlprofils, beziehungsweise einer dazu proportionalen Größe (siehe Abschnitt (REF!)), erfolgt mithilfe eines Messschiebers, der in den Strahlengang gebracht wird.
...
Dann wird dessen Abstand $z$ vom ebenen Resonatorspiegel langsam vergrößert und beobachtet, wann der Strahl nichtmehr aufleuchtet, wenn der Messschieber um die Strahlachse bewegt wird.
...
Daher wird der statistische Fehler der Spaltbreite konservativ auf $\Delta d = \SI{0.01}{\milli\metre}$ abgeschätzt.
...
Diese Verluste führen zu einer Änderung der Schwellenbedingung für Laseroperation, was bei dieser Messmethode ausgenutzt wird, um eine zum Strahlradius $w$ proportionale Größe $d$ bzw. $d/2$ zu messen.
