...
\begin{table}[h]
	...
	\caption{Untergrundmessung von \isotope[137]{Ba} bei 1\% und 4\% Transmission}
...
Mit den angepassten Linienschwerpunkten kann die Impulskaltbration nun durchgeführt werden.
Dazu wurden in Abschnitt \ref{ssec:kalibration} bereits die zu den Linien gehörigen $(B \rho)$-Werte berechnet.
...
Diese Form des Graphens wurde gewählt, da bei der Anpassung einer Ursprungsgeraden mit \texttt{gnuplot} nur $y$-Fehler beachtet werden können.
\begin{figure}[h]
	\centering
...
Nachdem die Kalibration abgeschlossen wurde, sollen die $\beta$-Spektren von \isotope[204]{Tl} und \isotope[22]{Na} bei Transmissionen von \SI{4}{\percent} vermessen werden.
Die Messzeit bei dieser Transmission beträgt \SI{100}{\second}.
...
\begin{table}[h]
	...
	\caption{Untergrund Thallium (WRAPFIG?)}
...
Zur Bestimmung der Endpunktsenergie $\epsilon_0$ mit dem Kurie-Plot muss zunächst die Fermi-Funktion von Thallium geeignet interpoliert werden.
Dazu passen wir gemäß der Hypothese \cite{fermi_function}:
\begin{align*}
	F(Z, \epsilon) = \sqrt{A + \frac{B}{\epsilon - 1}}
\end{align*}
eine Kurve an die Datenpunkte von \cite{anleitung} an.
Die Anpassung ergibt:
\begin{align*}
	A &= \num{137.6 +- 3.5 } \\
	B &= \num{348.93 +- 0.79}
\end{align*}
und ist zusammen mit den gegebenen Datenpunkten in Abbildung \ref{fig:fermi_tl} dargestellt.
\begin{figure}[h]
	\centering
...
\begin{table}[h]
	...
	\caption{Untergrund Natrium (WRAPFIG?)}
...
\begin{sidewaystable}
	...
	\caption{Messdaten des Spektrums von \isotope[204]{Tl} bei einer Transmission $T = \SI{4}{\percent}$ und Messdauer $t = \SI{100}{s}$.}
