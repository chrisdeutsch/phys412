...
Die Leistungsverluste an der Diode (z.B. Reflexionen an der Vorderseite der $\lambda/4$-Platte und des Polarisators) belaufen sich somit auf ca. \SI{1}{\milli\watt}.
...
Die angesprochenen Reflexionen an der optischen Diode führen zu einer unerwünschten Rückkopplung des Strahls in den Resonator, weswegen die $\lambda/4$-Platte und der Polarisator leicht gegen die optische Achse verdreht wurden, damit der Strahl nicht in sich selbst reflektiert wird.
...
Abhilfe würde hier ein bessere Wechselspannungsquelle schaffen.
...
\caption{Messwerte und Berechnung zum longitudinalen Modenabstand für die Resonatorlänge \SI{69+-0.4}{\centi\meter} (Datensatz ALL0026). $x_i$ beschreibt die Position der Maxima, $\Delta T$ die Differenz zweier aufeinander folgender Positionen und $T_\text{konf.}$ den Abstand zweier gleicher Tranversalmoden. Dieser ist getrennt dargestellt, da die Werte keinen zeilenweisen Bezug zu den $x_i$ haben.}
\end{table}
\clearpage
\begin{figure}[ht]
\centering
...
\caption{Messwerte und Berechnung zum longitudinalen Modenabstand für die Resonatorlänge \SI{79.5+-0.4}{\centi\meter} (Datensatz ALL0022). $x_i$ beschreibt die Position der Maxima, $\Delta T$ die Differenz zweier aufeinander folgender Positionen und $T_\text{konf.}$ den Abstand zweier gleicher Tranversalmoden. Dieser ist getrennt dargestellt, da die Werte keinen zeilenweisen Bezug zu den $x_i$ haben.}
\end{table}
\clearpage
\begin{figure}[ht]
\centering
...
\caption{Messwerte und Berechnung zum longitudinalen Modenabstand für die Resonatorlänge \SI{89.2+-0.4}{\centi\meter} (Datensatz ALL0014). $x_i$ beschreibt die Position der Maxima, $\Delta T$ die Differenz zweier aufeinander folgender Positionen und $T_\text{konf.}$ den Abstand zweier gleicher Tranversalmoden. Dieser ist getrennt dargestellt, da die Werte keinen zeilenweisen Bezug zu den $x_i$ haben.}
\end{table}
\clearpage
