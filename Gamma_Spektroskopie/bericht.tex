...
Aus Gründen der Übersichtlichkeit wurde in Abbildung \ref{fig:abzug_untergrund} der Abzug des Untergrundes am Beispiel der Messung von \isotope[60]{Co} durchgeführt und bilden folglich nur noch die vom Untergrund bereinigten Spektren ab.
Dabei wird für den Fehler in der Anzahl der Ereignisse $N$ eines Kanals $n$ wegen der zugrundeliegenden Poisson-Verteilung
\begin{align}
\Delta N = \sqrt{N}
\end{align}
angenommen.
Wird der Untergrund vom Spektrum abgezogen, so folgt für den Fehler des untergrundkorrigierten Spektrums gemäß der Gauß'schen Fehlerfortpflanzung:
\begin{align}
\label{eq:untergrundkorr_fehler}
N_\mathrm{korr.} = N - N_\mathrm{Untergrund} \qquad \Delta N_\mathrm{korr.} = \sqrt{\Delta N^2 + \Delta N_\mathrm{Untergrund}^2}
\end{align}
