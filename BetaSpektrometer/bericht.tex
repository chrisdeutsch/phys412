...
Dazu wird ein doppeltfokussierendes $\beta$-Spektrometer verwendet, womit zunächst eine Impulskalibation anhand der Konversionslinien von \ce{^{137}Cs} durchgeführt wird.
...
\subsubsection{Bestimmung der Maximalenergie von Thallium (\ce{^{204}Tl})}
...
\subsubsection{Bestimmung der Endpunktsenergie von Natrium (\ce{^{22}Na})}
...
\FloatBarrier
\section{Fazit}
Insgesamt ist der hier behandelte Versuch nur bedingt gut gelungen.
Prinzipiell konnten alle Messungen ohne größere Schwierigkeiten durchgeführt werden, jedoch waren die gemessenen Werte HIER ERKLÄRUNG WEIL SCHEISSE.
Die Impulskalibration gelang mit den aufgenommenen Konversionslinien von Caesium relativ gut, wobei die Messung mit \SI{1}{\percent} Transmission nicht die erwartete Verbesserung der Auflösung brachte.
Die Bestimmung der Endpunktsenergie von Natrium gelang gut, die bestimmte Maximalenergie von Thallium weicht jedoch stark vom Literaturwert ab, wofür keine konkrete Ursache in der Auswertung gefunden werden konnte (siehe auch das Fazit in \ref{sssec:kurie_thallium}).\\
\\
Dennoch konnte in dem Versuch ein guter Einblick in Aufbau und Funktionsweise eines Spektrometers sowie in die Arbeitsweise der $\beta$-Spektroskopie gewonnen werden.
