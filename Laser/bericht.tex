...
\end{align}
\textbf{Was bedeutet $\varphi$. Das ist nicht der Winkel am Polarisator..}
Eigentlich misst man:
\begin{align}
	I = I_0 \cdot \cos^2 (\varphi - \varphi_0)
\end{align}
oderso.
Dann ist $\varphi$ tatsächlich der Polarsiatorwinkel und $\varphi_0$ die Polarisationsrichtung. 
...
\begin{table}[h]
	\centering
	\input{./tables/strahlradien/80cm.tex}
...
\begin{table}[h]
	\centering
	\input{./tables/strahlradien_fit_resultate.tex}
...
Dazu wird ein externer konfokaler Resonator genutzt, dessen Transmission maximal wird, wenn die Frequenz des Lasers \textbf{eher der jeweiligen Mode} in Resonanz mit der Frequenz des exteren Resonator ist.
Die Länge des Resonators wird mit einer anliegenden Wechselspannung \textbf{Rampe ist hier wichtig damit die gemessenen Zeitdifferenzen auf dem Oszi auch proportional zur Spannungsdifferenz und damit zur Längendifferenz sind} kontinuierlich verändert und die Intensität mit einer Photodiode in Abhängigkeit der Länge des Resonators (die proportional zur angelegten Spannung ist), beobachtet.
...
Dies ist gegeben, wenn auf dem Oszilloskop zwei gaußförmige Einhüllende über dem Modenspektrum (das aus Lorentzkurven aufgebaut ist) zu beobachten sind. \textbf{hier explizit die Fragestellung in dick auf dem Blatt beantworten}
...
Die dabei gemessenen Verläufe wurden in den Abbildungen \ref{fig:ALL0028} bis \ref{fig:ALL0014} gezeigt. \textbf{Immer ein Bild in die Auswertung. Rest in Anhang.}
...
Diese folgt aus einer \emph{least-squares}-Anpassung \textbf{Methode kleinster Quadrate} mit \texttt{Gnuplot} auf Basis einer Superposition von $n$ Lorentzprofilen:
\begin{align}
f(x)=\sum_{i=1}^{n}a_i\cdot\frac{b_i^2}{(x-c_i)^2+b_i^2}
\end{align}
Dabei beschreibt $a_i$ die Höhe, $b_i$ die Breite und $c_i$ die Lage des $i$-ten Peaks. \textbf{Anderes deutsches Wort für Peaks}
...
\begin{figure}[htb]
\centering
\input{./plots/modenabstand/ALL0028.tex}
\caption{Resonatorlänge 50cm ALL0028}
\label{fig:ALL0028}
\end{figure}
\begin{figure}[htb]
\centering
\input{./plots/modenabstand/ALL0026.tex}
\caption{Resonatorlänge 70cm ALL0026}
\end{figure}
\label{fig:ALL0026}
\begin{figure}[htb]
\centering
\input{./plots/modenabstand/ALL0022.tex}
\caption{Resonatorlänge 80cm ALL0022}
\label{fig:ALL0022}
\end{figure}
\begin{figure}[htb]
\centering
\input{./plots/modenabstand/ALL0014.tex}
\caption{Resonatorlänge 90cm ALL0014}
\label{fig:ALL0014}
\end{figure}
\FloatBarrier
\subsubsection{Bestimmung der Modenabstände}
Da die Messung der Frequenzabstände zweier Moden in Einheiten des konfokalen Modenabstands durchgeführt werden soll, muss dieser zunächst berechnet werden. \textbf{Nein muss er nicht. Erst wenn man die Frequenzabstände in Frequenzeinheiten haben will.}
Er ergibt sich aus der Überlegung, dass in einem konfokalen Resonator das einfallende Licht intern dreimal reflektiert wird und der Strahl so den vierfachen Spiegelabstand durchläuft, bis er an der Stelle, an der er eintrat, austritt. \textbf{Erklärung weg einfach nur auf Siegmann verweisen.}
EVTL GRAFIK
...
\end{align}
Daraus folgt für den freien Spektralbereich des konfokalen Resonators \textbf{Vllt. eine Fußnote zu der Unsicherheit was FSR und konfokaler Modenabstand angeht und wie wir ihn letztendlich definiert haben.} (siehe auch \eqref{eq:modenabstand}):
\begin{align}
...
Im konfokalen Modenabstand von einer longitudinalen Mode (d.h. genau in der Mitte des freien Spektralbereichs) treten im Fall des konfokalen Resonators die von Siegman \cite{siegman} als \emph{half axial modes}, im Folgenden \emph{halbaxiale Moden} genannt auf. \textbf{Lieber erklären woher die kommen (ungerade $n+m$ fallen die Transversalmoden genau zwischen die Axialmoden) statt die Benennung von Siegmann zitieren.}
Die zwei beobachteten Gaußpeaks entpechen daher einer longitudinalen und einer halbaxialen Mode.\\
\\
...
In Tabelle \ref{tab:ALL0028} wurden dabei die jeweiligen Positionen $x_i$ der Maxima (Ergebnisse der Anpassung) notiert und die Abstände $\Delta T$ zweier aufeinanderfolgender Maxima innerhalb eines Gaußpeaks berechnet.
Der Fehler ergibt sich mit Gaußscher Fehlerfortpflanzung analog zu \eqref{eq:fehler_differenz} mit:
\begin{align}
\Delta(\Delta T)=\Delta(x_{i+1}-x_i)=\sqrt{(\Delta x_{i+1})^2+(\Delta x_i)^2}
\end{align}
Außerdem wurde der Abstand zweier gleicher Moden (d.h. für den Fall von fünf Moden pro Gaußpeak der Abstand $T_\text{kon}=x_{i+5}-x_i$) \textbf{vllt was allgemeiner ausdrücken und in der grafik markieren?} errechnet, der Fehler berechnet sich wie oben. \textbf{Versteh ich nicht.}
Dieser Wert entspricht gerade dem konfokalen Modenabstand. \textbf{Das auch nicht. Vermutlich wegen der unklaren Definition von gleichen Moden. Habs doch verstanden ist aber vllt. nicht ganz klar ausgedrückt.}
...
\input{./tables/modenabstand/ALL0028.tex}}
\caption{Messwerte und Berechnung zum longitudinalen Modenabstand. $x_i$ beschreibt die Position der Maxima, $x_{ij}$ die Differenz zweier aufeinander folgender Positionen \textbf{Notationsänderung? seh ich nicht in der tab.} und $T_\text{kon}$ den Abstand zweier gleicher Tranversalmoden. Diese sind getrennt dargestellt, da sie keinen zeilenweisen Bezug zu den $x_i$ haben.\textbf{punkt zu komma}}
\label{tab:ALL0028}
\end{table}
Mit den Mittelwerten kann nun der Abstand zweier Moden in Einheiten des konfokalen Modenabstands gefunden werden.
Man erhält so für die hier betrachtete Resonatorlänge \SI{50+-0.4}{\centi\metre} \textbf{Hier müsste noch erklärt werden, dass das auch für die Frequenzen und nicht nur für die Oszizeiten gilt.}:
\begin{align}
\frac{\Delta T}{T_\text{kon}}= \num{0.201+-0.001}
\end{align}
Aus der Gaußschen Fehlerfortpflanzung folgt für den Fehler:
\begin{align}
\Delta\left(\frac{\Delta T}{T_\text{kon}}\right)=\sqrt{\frac{(\Delta(\Delta T))^2}{T_\text{kon}^2}+\frac{(\Delta T)^2(\Delta T_\text{kon})^2}{T_\text{kon}^4}}
\end{align}
Die Berechnung des Modenabstands erfolgt nach:
\begin{align}
\Delta\nu_\text{long}=\frac{\Delta\nu_\text{kon}}{T_\text{kon}} \cdot \Delta T 
\end{align}
...
\begin{align}
\Delta\nu_\text{long}=\SI{301.3+-0.8}{\mega\hertz}
\end{align}
Der Fehler wurde abermals mit Gaußscher Fehlerfortpflanzung und mit den einzelnen Fehlern für $\Delta T$ und $T_\text{kon}$ berechnet mit:
\begin{align}
\Delta\left(\Delta\nu_\text{long}\right)=\Delta\nu_\text{kon}\cdot\sqrt{\frac{(\Delta(\Delta T))^2}{T_\text{kon}^2}+\frac{(\Delta T)^2(\Delta T_\text{kon})^2}{T_\text{kon}^4}}
\end{align}
\textbf{Ich würde in den Indizes konfokal mit konf. abkürzen. kon sieht irgendwie komisch aus. Außerdem würde ich auch in den Indizes bei Abkürzungen Punkte machen. So hab ich das bisher auch gemacht.}
...
\begin{table}[h]
	\centering
	\input{./tables/optische_schwebung.tex}
...
\begin{table}[b]
	...
	\caption{Messwerte der Spannung an der Photodiode in Abhängigkeit des Winkels am Linearpolarisator \textbf{Falsche Anzahl an stellen für Photospannung/Fehler. Und bezeichnung von Photospan./Fehler inkonsistent.}}
	\label{tab:malus}
\end{table}
\FloatBarrier
...
\begin{table}[ht]
	\centering
	\input{./tables/strahlradien/50cm.tex}
...
\begin{table}[t]
	\centering
	\input{./tables/strahlradien/70cm.tex}
...
\begin{table}[t]
	\centering
	\input{./tables/strahlradien/90cm.tex}
