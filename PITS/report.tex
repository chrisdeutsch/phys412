...
Since all Keplerian elements but true anomaly are not dependent on time they can be looked up, see for example \url{http://nssdc.gsfc.nasa.gov/planetary/factsheet/earthfact.html}.
True anomaly itself can not be easily computed so we did a linear approximation based on the data \url{http://www.wolframalpha.com} gives for the time period of observation which is short compared to earth's revolution time so that a linear approximation for $\nu(t)$ should be sufficient for our needs.
...
After transforming both earth's position (to be precise the position of the center of the earth) from Keplerian elements and the observatory's position from spherical to spacial coordinates and velocity the time-dependent state vector of the observer can be calculated.
