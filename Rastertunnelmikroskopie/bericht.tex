...
\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{./grafiken/ref_graphit_pt_ir_1nm_laenge.png}
...
\end{equation}
Der Fehler einer Einzelmessung ist dabei gegeben durch die Standardabweichung $\sigma_\mathrm{Bind.}$:
\begin{equation}
  \sigma_\mathrm{Bind.} = \sqrt{\frac{1}{N-1} \sum_{i=1}^N (d_i - \bar{d}_\mathrm{Bind.})^2} = \SI{0,15}{\angstrom}
\end{equation}
und folglich der Fehler des Mittelwerts als:
\begin{equation}
  \Delta \bar{d}_\mathrm{Bind.} = \frac{\sigma_\mathrm{Bind.}}{\sqrt{N}} = \SI{0,03}{\angstrom}
\end{equation}
