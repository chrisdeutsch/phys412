...
Das Coulombfeld des Kerns wirkt anziehend auf Elektronen, so dass das Spektrum im Bereich kleiner Impulse eine Erhöhung im Gegensatz zum nicht korrigierten Spektrum aufweist, für Positronen analog eine Abflachung.
Dieses Phänomen kann durch Multiplikation des Spektrums mit der s.g. Fermifunktion $F(Z, p)$ berücksichtigt werden.\\
\\
...
Zur Vereinfachung werden folgende dimensionslose Größen eingeführt:
\begin{align}
	\epsilon = 1 + \frac{E_\mathrm{kin}}{m_\mathrm{e} c^2} \\
	\eta = \frac{p}{m_\mathrm{e} c} \\
	\epsilon_0 = 1 + \frac{Q}{m_\mathrm{e} c^2} \\
\end{align}
Das Spektrum aus \eqref{eq:spektrum_krane} kann man mit
\begin{align}
N(\epsilon) = N(p) \frac{\mathrm{d}p}{\mathrm{d}\epsilon}\propto N(p) \frac{\epsilon}{\eta}
\end{align}
als Funktion der Energie schreiben und man erhält durch Umstellen die benötigte Form, die im Kurieplot aufgetragen wird:
\begin{align}
...
Unter Freisetzung von Energie kann dann ein elektron aus einer äußeren Schale diesen Platz einnehmen und die dabei entstehende Energie auf ein weiteres Elektron übertragen werden, was dadurch seine Bindungsenergie überwindet und frei wird.
...
Die dabei entstehenden Linien, die in diesem ersuch allerdings nicht beobachtet werden können, sind meist bei geringen Impulsen am linken Rand des Spektrums zu finden.\\
\\
Im Gegensatz dazu steht das Phänomen der inneren Konversion.
Dabei geht ein angeregtes Atom in einen niedrigeren Zustand über, wodurch Energie frei wird.
Diese geht dann auf ein Elektron in der Atomhülle über, was dadurch seine Bindungsenergie überwindet und das Atom verlässt.
Die Energie ist genau die Differenz zwischen freigesetzter Energie des Atoms und der Bindungsenergie des Elektrons.
Da die Bindungsenergie der einzelnen Schalen unterschiedlich ist, erhält man für jede Schale eine (oder ggf. mehrere, siehe Auswertung) charakteristische Linie im Zerfallsspektrum.
...
Vor Inbetriebnahme des Magneten muss die Wasserkühlung aktiviert werden, erst danach kann der Strom am Drehregler des Transformators eingeschaltet werden.
Das während der Messung am Detektor registrierte Signal wird anschließend in ein logisches Signal umgewandelt, welches in eine Zähleinheit mit einstellbarer Messzeit geleitet wird, an der auf einer LED Matrix die Zählrate abgelesen werden kann.
...
Um die tatsächliche Offsetspannung zu ermitteln, nutzt man die Hysterese des Magneten aus und misst beim Durchlaufen der Hystereseschleife, die jeweiligen Remanenzfelder (als Hallspannung $U_\text{H}$) bei abgeschaltetem Magnetstrom.
...
\begin{align}
	\overline{U}_0 = \SI{7.16 +- 0.03}{\skt}
...
\begin{table}[h]
	...
	\caption{Relative Linienbreiten bei verschiedenen Transmissionen als Kenngröße für das Auflösungsvermögen. $\overline{R}$ kennzeichnet den fehlergewichteten Mittelwert der Linien bei konstanter Transmission.}
