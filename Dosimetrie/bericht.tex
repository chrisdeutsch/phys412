...
\begin{align}
	\Delta R &= \sqrt{\frac{(\Delta N)^2}{T^2}} \qquad \Delta N = \sqrt{N} \\
				 ...
				 &= \sqrt{\frac{R}{T}}
\end{align}
Grafisch aufgetragen erhält man aus diesen die Abbildung REF
Es ist deutlich zu sehen, dass es sich bei dem beobachteten Aufbau um ein nicht-paralysierendes System handelt, wobei bei genauer Betrachtung ein geringer paralysierender Anteil gefunden werden kann.
Einen Hinweis dafür liefert die Tatsache, dass die Zählrate nicht wie für ein nicht-paralysierendes System erwartet in Sättigung geht, sondern nach Erreichen eines Maximums wieder leicht zurückgeht bei zunehmendem Emissionsstrom.
Da dieser Effekt nur sehr gering ist im Vergleich zu dem Verlauf eines rein nicht-paralysierenden Systems kann für alle folgenden Messungen mit dem verwendeten Zählrohr von einem nicht-paralysierenden System ausgegangen werden.
...
\begin{table}[hp]
	...
	\caption{Zusammenhang von Ionisationsstrom $I_\mathrm{C}$, welcher anhand des Spannungsabfalls $U_{I_\mathrm{C}}$ an einem Messwiderstand $R = \SI{1+-0.01}{\giga\ohm}$ gemessen wurde und Kondensatorspannung $U_\mathrm{C}$. Die Röntgenröhrenspannung der Molybdän-Röhre wurde auf $U = \SI{15}{kV}$ eingestellt.}
\end{table}
\begin{table}[hp]
	\centering
	\input{./tables/kondensator/25kV.tex}
	\label{tab:kondensatorspannung_25kV}
	\caption{Zusammenhang von Ionisationsstrom $I_\mathrm{C}$, welcher anhand des Spannungsabfalls $U_{I_\mathrm{C}}$ an einem Messwiderstand $R = \SI{1+-0.01}{\giga\ohm}$ gemessen wurde und Kondensatorspannung $U_\mathrm{C}$. Die Röntgenröhrenspannung der Molybdän-Röhre wurde auf $U = \SI{25}{kV}$ eingestellt.}
\end{table}
\begin{table}[hp]
	\centering
	\input{./tables/kondensator/35kV.tex}
	\caption{Zusammenhang von Ionisationsstrom $I_\mathrm{C}$, welcher anhand des Spannungsabfalls $U_{I_\mathrm{C}}$ an einem Messwiderstand $R = \SI{1+-0.01}{\giga\ohm}$ gemessen wurde und Kondensatorspannung $U_\mathrm{C}$. Die Röntgenröhrenspannung der Molybdän-Röhre wurde auf $U = \SI{35}{kV}$ eingestellt.}
\end{table}
\FloatBarrier
