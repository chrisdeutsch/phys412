...
Zusätzlich dazu kann man weitere Quantenzahlen definieren, wir beschränken uns hier auf die Quantenzahlen $\ell$ für den Drehimpuls, $s$ für den Spin und $j$ für den Gesamtdrehimpuls aus $\ell$ und $s$.
Für $n$ und $\ell$ gilt $n=1, 2, 3 \dots$ und $\ell=0, 1, 2 \dots n-1$, wobei die Werte für $\ell$ häufig mit den Buchstaben s, p, d, f, g $\dots$ (ab f alphabetisch) identifiziert werden.
Die möglichen Werte von $j=\ell+s$ ergeben sich mit dem Elektronenspin $s=\pm1/2$ entsprechend.
Da es nicht möglich ist, zwei Komponenten $L_i, L_j$ des Drehimpulses gleichzeitig zu bestimmen (folgt aus Kommutatorrelation), erhält man bei einer Messung von $|\vec{L}|$ und $L_z$, welche durchgeführt werden kann, eine weitere Quantenzahl $m_\ell$ (analog erhält man für $|\vec{J}|$ und $J_z$ $m_j$). Sie beschreibt im Vektormodell anschaulich die Projektion von $\vec{L}$ auf die $z$-Achse.
...
\begin{subfigure}[l]{0.5\textwidth}
	...
	\subcaption{}
\end{subfigure}
\caption{Anpassung an die aufgespaltete Cadmium-Linie im Magnetfeld für \textbf{(a)} \SI{8}{\ampere}, \textbf{(b)} \SI{8.5}{\ampere}, \textbf{(c)}) \SI{8.9}{\ampere}}
\end{sidewaysfigure}
