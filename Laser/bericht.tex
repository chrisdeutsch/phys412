...
Der Fehler $\Delta\varphi$ für den Winkel ergibt sich aus der Tatsache, dass der Polarisator in Schritten von \SI{2}{\degree} eingestellt werden konnte, der Fehler für die Intensität wurde aufgrund der Schwankungen, die zu beobachten waren (es wurde mit der \emph{Measure}-Funktion des Oszilloskops gemessen), auf \SI{0.2}{\milli\volt} abgeschätzt.
\begin{table}
	\centering
	...
	\label{tab:malus}
\end{table}
Diese Messwerte werden im Anschluss in ein Diagramm eingetragen und mit \texttt{Gnuplot} eine Kurve der Form
\begin{align}
...
Die Abweichungen der Anpassung (das reduzierte $\chi^2$ von \num{57.1} bestätigt die schlechte Übereinstimmung) von den Messwerten lassen sich durch eine nicht ideale Justage erklären.
Der Laserstrahl trifft dabei nicht das Zentrum des Polarisators, so dass die Winkelabhängigkeit der Intensität nicht optimal gemessen werden kann.
