...
Im konfokalen Modenabstand von einer longitudinalen Mode (d.h. genau in der Mitte des freien Spektralbereichs) treten im Fall des konfokalen Resonators die von Siegman \cite{siegman} als \emph{half axial modes}, im Folgenden \emph{halbaxiale Moden} genannt auf.
...
\end{align}
Außerdem wurde der Abstand zweier gleicher Moden (d.h. für den Fall von fünf Moden pro Gaußpeak der Abstand $T_\text{kon}=x_{i+5}-x_i$) errechnet, der Fehler berechnet sich wie oben.
...
\caption{Messwerte und Berechnung zum longitudinalen Modenabstand. $x_i$ beschreibt die Position der Maxima, $x_{ij}$ die Differenz zweier aufeinander folgender Positionen und $T_\text{kon}$ den Abstand zweier gleicher Tranversalmoden. Diese sind getrennt dargestellt, da sie keinen zeilenweisen Bezug zu den $x_i$ haben.}
\label{tab:ALL0028}
\end{table}
Mit den Mittelwerten kann nun der Abstand zweier Moden in Einheiten des konfokalen Modenabstands gefunden werden.
Man erhält so für die hier betrachtete Resonatorlänge \SI{50+-0.4}{\centi\metre}:
\begin{align}
