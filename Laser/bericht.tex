...
In Tabelle \ref{tab:ALL0028} wurden dabei die jeweiligen Positionen $x_i$ der Maxima (Ergebnisse der Anpassung) notiert und die Abstände $\Delta x_{ij}$ zweier aufeinanderfolgender Maxima innerhalb eines Gaußpeaks berechnet.
Der Fehler ergibt sich mit Gaußscher Fehlerfortpflanzung analog zu \eqref{eq:fehler_differenz} mit:
\begin{align}
\Delta(\Delta x_{ij})=\Delta(x_{i+1}-x_i)=\sqrt{(\Delta x_{i+1})^2+(\Delta x_i)^2}
\end{align}
Außerdem wurde der Abstand zweier gleicher Moden (d.h. für den Fall von fünf Moden pro Gaußpeak der Abstand $x_{i+5}-x_i$) errechnet, der Fehler berechnet sich wie oben.
...
Der konfokale Modenabstand (hier \SI{1.5}{\giga\hertz}) ist bekannt und kann so als Ausgangspunkt zur Umrechnung zwischen Frequenz- und Zeitdomäne genutzt werden.
